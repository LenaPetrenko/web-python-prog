...
def parse_cdp_neighbors(command_output):
    ...
    for line in command:
        if line.strip() != '' :
            ...
            if ForReadind :
                line_for_work = line.split()
                device_id = line_for_work[0]
                local_Intrfce = line_for_work[1]+line_for_work[2]
                Port_id = line_for_work[-2]+line_for_work[-1]
                dictionary[device, local_Intrfce] = device_id, Port_id
            if line.find('Device ID') != -1:
                ForReadind = True
    return dictionary

if __name__ == "__main__":
    with open("/home/std/python-web/exercises/11_modules/sh_cdp_n_sw1.txt") as f:
